...
def test_require_valid_github_token(monkeypatch):
    ...
    assert app.require_github_token().startswith("ghp_")
    monkeypatch.setenv("GITHUB_TOKEN", "github_pat_" + "b" * 40)
    assert app.require_github_token().startswith("github_pat_")
    monkeypatch.setenv("GITHUB_TOKEN", "invalid_token")
    with pytest.raises(SystemExit):
        app.require_github_token()
